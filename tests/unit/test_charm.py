...
@pytest.fixture
def harness():
    harness = ops.testing.Harness(LegoOperatorCharm)
    harness.begin()
    yield harness
    harness.cleanup()


def test_httpbin_pebble_ready(harness: ops.testing.Harness[LegoOperatorCharm]):
...
def test_config_changed_valid_can_connect(harness: ops.testing.Harness[LegoOperatorCharm]):
...
def test_config_changed_valid_cannot_connect(harness: ops.testing.Harness[LegoOperatorCharm]):
    # Trigger a config-changed event with an updated value
    harness.update_config({"log-level": "debug"})
...
def test_config_changed_invalid(harness: ops.testing.Harness[LegoOperatorCharm]):
    # Ensure the simulated Pebble API is reachable
    harness.set_can_connect("httpbin", True)
    # Trigger a config-changed event with an updated value
