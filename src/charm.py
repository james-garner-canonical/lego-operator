...
class LegoCharm(CharmBase):
    ...
    def _configure(self, event: EventBase) -> None:
        ...
        if err := self._validate_plugin_config_options():
            ...
            return
        self._ensure_account_private_key()
    ...
    def _ensure_account_private_key(self):
        """Ensure the account private key is available."""
        if self._private_key_stored():
    ...
    def _generate_and_store_private_key(self) -> None:
        """Generate a new private key and store it in a juju secret."""
        private_key = generate_private_key()
        try:
            secret = self.model.get_secret(label=self._account_private_key_secret_label)
            secret.set_content({"private-key": str(private_key)})
            secret.get_content(refresh=True)
        except SecretNotFoundError:
            self.unit.add_secret(
                content={"private-key": str(private_key)},
                label=self._account_private_key_secret_label,
            )

    def _private_key_stored(self) -> bool:
    ...
    def private_key(self) -> str | None:
        ...
        if not self._private_key_stored():
            return None
        secret = self.model.get_secret(label=self._account_private_key_secret_label)
